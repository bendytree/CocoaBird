...
{
    return [self processRequestSynchronous:@"http://api.twitter.com/1/saved_searches.json" method:@"GET" params:nil type:CBTwitterResponseTypeArray class:[CBSavedSearch class] error:error];
}

+ (NSString*) getSavedSearches:(id)delegate selector:(SEL)selector
{
    return [self processRequestAsynchronous:@"http://api.twitter.com/1/saved_searches.json" method:@"GET" params:nil type:CBTwitterResponseTypeArray class:[CBSavedSearch class] delegate:delegate selector:selector];    
}
...
{
    NSString* url = [NSString stringWithFormat:@"http://api.twitter.com/1/saved_searches/show/%i.json", search_id];
...
{
    return [self processRequestSynchronous:@"http://api.twitter.com/1/saved_searches/create.json" method:@"POST" params:nil type:CBTwitterResponseTypeObject class:[CBSavedSearch class] error:error];
}

+ (NSString*) createSavedSearch:(NSString*)query delegate:(id)delegate selector:(SEL)selector
{
    return [self processRequestAsynchronous:@"http://api.twitter.com/1/saved_searches/create.json" method:@"POST" params:nil type:CBTwitterResponseTypeObject class:[CBSavedSearch class] delegate:delegate selector:selector];    
}
...
{
    NSString* url = [NSString stringWithFormat:@"http://api.twitter.com/1/saved_searches/destroy/%i.json", search_id];
